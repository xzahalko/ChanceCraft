    ...
    public class ChanceCraftPlugin : BaseUnityPlugin
    {
        ...
        void Awake()
        {
            ...
            if (_loggingEnabled.Value)
                Logger.LogDebug("[ChanceCraft] Awake called!");
            Game.isModded = true;
        }
                ...
                if (isUpgradeCall || IsUpgradeOperation(gui, selectedRecipe) || _isUpgradeDetected)
                {
                    ...
                    try
                    {
                        ...
                        lock (typeof(ChanceCraftPlugin))
                        {
                            ...
                            {
                                if (_loggingEnabled.Value)
                                    BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogDebug("Performing UI refresh after revert attempts");
                            ...
                            {
                                if (_loggingEnabled.Value)
                                    BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogWarning($"UI refresh after revert failed: {exRefresh}");
                            }
        ...
        private static void DumpInventoryState(string tag, Player player, int max = 64)
        {
            try
            {
                ...
                {
                    if (_loggingEnabled.Value)
                        BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogDebug($"{tag}: player is null");
                    return;
                }
                var inv = player.GetInventory();
                if (inv == null)
                {
                    if (_loggingEnabled.Value)
                        BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogDebug($"{tag}: player inventory is null");
                    return;
                }

                var items = inv.GetAllItems();
                // Short summary by default
                BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogInfo($"{tag}: inventory contains {items?.Count ?? 0} items");
                
                // Full dump only when logging enabled
                if (_loggingEnabled.Value)
                {
                    BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogDebug($"{tag}: detailed dump (showing up to {max}):");
                    int i = 0;
                    foreach (var it in items)
                    {
                        ...
                        {
                            BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogDebug($"{tag}: item[{i - 1}] = <null>");
                            continue;
                        }
                        BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogDebug($"{tag}: item[{i - 1}] hash={it.GetHashCode():X8} name='{it.m_shared.m_name}' q={it.m_quality} v={it.m_variant} stack={it.m_stack}");
                    }
                }
            }
            catch (Exception ex)
            {
                BepInEx.Logging.Logger.CreateLogSource("ChanceCraft").LogWarning($"DumpInventoryState('{tag}') exception: {ex}");
            }
        }
